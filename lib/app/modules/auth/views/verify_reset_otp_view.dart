...
class VerifyResetOtpView extends StatefulWidget {
  final String email;

  VerifyResetOtpView({required this.email});

  @override
  _VerifyResetOtpViewState createState() => _VerifyResetOtpViewState();
}

class _VerifyResetOtpViewState extends State<VerifyResetOtpView> {
  ...
  void _initializeCountdown() async {
    ...
    int remaining = await authController
        .getRemainingOtpCountdown('reset_otp_countdown_end');
    if (remaining > 0) {
      setState(() {
        countdown = remaining;
  ...
  void _resendOtp() async {
    final authController = Get.find<AuthController>();
    await authController.resendResetOtp(widget.email);
    int remaining = await authController
        .getRemainingOtpCountdown('reset_otp_countdown_end');
    if (remaining > 0) {
      setState(() {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text('Verifikasi Reset Password'),
        centerTitle: true,
          ...
          children: [
            ...
            ElevatedButton(
              onPressed: () {
                ...
                authController.verifyResetOtp(widget.email, otp);
              },
